...
import { getClientProjectsApi } from "@/services/client/project";
import DisplayClientRatings from "../client/ratings/get-ratings";
...
function GetClientDetails() {
  ...
  useEffect(() => {
    async function load() {
      const res = await getClientProjectsApi(id);
  ...
  useEffect(() => {
    async function load() {
      try {
        const detailsRes = await getDetailsByUserIdApi(id);
      ...
      <div className="flex flex-row items-start justify-center gap-4 pt-4 pb-30 bg-gradient-to-br from-blue-100 to-indigo-100">
        <div className="flex flex-col relative w-[890px] rounded-b-sm space-y-4">
          <div className="flex flex-row rounded-xl shadow-[0_0_10px_rgba(0,0,0,0.1)] p-6 bg-blue-50">
            ...
            <button
              className="bg-blue-600 font-semibold text-white px-4 py-2"
              onClick={() => navigate(`/freelancer/chat/${details.createdBy}`)}
            >
              Message
            </button>
          </div>
