...
import acceptBidApi from "@/services/client/bid-status/accepted";
...
function ProjectProposals({ projectId }) {
  ...
  return (
    <div className="flex flex-col mt-4 justify-center">
      <p className=" flex flex-col absolute right-95 items-center top-36">
        <span className="text-[13px]">Bids</span>
        <span className="text-[22px] font-bold -mt-1">{proposals.length}</span>
      </p>
      <div className="w-full">
        <div className=" dark:bg-gray-900 flex flex-col dark:text-gray-100 space-y-2">
          {proposals.map((p) => (
            <div key={p._id || p.bidId} className="rounded-md ">
              <div className="flex flex-row  rounded-xl py-8 pl-8 pr-10 bg-gray-100 shadow-[0_0_6px_rgba(0,0,0,0.2)]">
                <div className="flex flex-col gap-2">
                    ...
                    <div className="flex flex-col space-y-1 mt-0.5">
                      <div className="flex flex-row ml-1">
                        <p
                          ...
                          onClick={() =>
                            navigate(
                              `/client/freelancer/profile/${p.freelancerId}`
                            )
                          }
                        >
                          {p.freelancerName}
                  ...
                  <div className="flex flex-col w-[950px] px-1 space-y-4">
                ...
                <div className="flex flex-col items-center justify-between w-44">
                  ...
                  <div className="mb-2">
                    {p.bidStatus === "pending" && (
                      <button
                        onClick={() => handleAccept(p.bidId)}
                        className="bg-gradient-to-r from-blue-500 to-indigo-500 hover:bg-gradient-to-r hover:from-blue-600 hover:to-indigo-600 text-white font-semibold px-4 py-2 rounded cursor-pointer"
                      >
                        Accept
                      </button>
                    )}
